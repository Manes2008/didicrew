...
1) Thời lượng, Số cảnh và Lượng từ: Mỗi cảnh trung bình phải dài 2-4 giây. Tổng số cảnh phải xấp xỉ {target_dur}/3.0. Tổng lượng từ voiceover của toàn bộ kịch bản phải xấp xỉ {target_dur} * 3.0 từ (ví dụ: video 40s cần khoảng 120 từ, video 60s cần khoảng 180 từ) để đảm bảo kịch bản đủ thông tin và kịch tính.
...
6) Gộp phân cảnh Veo3 & Tách thoại rõ ràng: Phần 3 (Veo3 Script) bắt buộc phải được gộp từ các cảnh ngắn 2-4s thành các Phân cảnh Veo3 lớn dài xấp xỉ 8 giây (Ví dụ: video 60s gộp thành khoảng 7-8 phân cảnh). Lời thoại/lời dẫn (Voiceover / Dialogue (VI)) trong mỗi phân cảnh Veo3 lớn phải phân tách rõ vai bằng thẻ [NARRATOR] hoặc [DIALOGUE - Tên nhân vật]. Tuyệt đối không để nhân vật tự đọc lời dẫn chuyện hay tự đọc lời cốt chuyện. TUYỆT ĐỐI KHÔNG ĐƯỢC ĐỂ TRỐNG VOICEOVER ở bất kỳ cảnh nào trong bảng phân cảnh Phần 2. Mỗi Phân cảnh Veo3 lớn (8s) ở Phần 3 bắt buộc phải đạt từ **18 đến 26 từ** voiceover (tốc độ đọc nhanh kịch tính ~3 từ/giây). Nếu phát hiện cảnh trống voiceover, hoặc phân cảnh Veo3 có thoại quá ngắn (dưới 12 từ), điểm bắt buộc phải dưới 8 để yêu cầu viết lại.
