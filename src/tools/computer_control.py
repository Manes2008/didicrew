...
def _base_dir() -> Path:
    # Trỏ về thư mục Mark-L song song để giữ cấu hình dùng chung
    return Path(__file__).resolve().parent.parent.parent.parent / "Mark-L"
...
def _get_api_key() -> str:
    # Ưu tiên lấy từ .env của VideoCrew
    import os
    env_key = os.getenv("GEMINI_API_KEY")
    if env_key:
        return env_key
    # Fallback về config của Mark-L
