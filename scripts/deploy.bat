...
docker compose down
echo [VideoCrew Docker] Dang build va khoi chay cac container moi (FastAPI Backend + PostgreSQL)...
docker compose up -d --build --force-recreate

if %errorlevel% == 0 (
    echo [VideoCrew Docker] Khoi chay thanh cong!
    echo [VideoCrew Docker] Backend FastAPI dang chay tai: http://localhost:8000 (Swagger: /docs)
